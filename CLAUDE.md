...
- **安全性（Security）**：`config/SecurityConfig.java` 提供一條 stateless HTTP Basic `SecurityFilterChain` — `anyRequest().authenticated()`，並放行 `actuator health/info` + springdoc（`/v3/api-docs/**`、`/swagger-ui/**`）。使用者為 in-memory（`api`、`internal`），憑證來自 `app.auth.*`（可用 env 覆寫）。此 security filter chain 只攔截 **inbound HTTP** — Oracle/Hikari、OTLP metric push 與 container 網路皆不受影響。由於 `*Client` 呼叫會透過 HTTP 迴繞回本 app，`RestClientConfig` 會附上 `internal` 帳號的 Basic 憑證，讓自我呼叫能通過 filter chain。另有一條 `@Profile("openapi")` 的 permit-all chain，使 `generateOpenApiDocs` 得以運作。
