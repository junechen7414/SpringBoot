...
- **Observability**: app exports metrics over OTLP (Micrometer) → Grafana Alloy → Prometheus → Grafana. Relevant infra files: `docker-compose.yml`, `config.alloy`, `prometheus.yml`. NB: monitoring is **OTLP push** (outbound) — there is no `/actuator/prometheus` scrape endpoint (no prometheus registry dependency).
- **Security**: `config/SecurityConfig.java` provides a stateless HTTP Basic `SecurityFilterChain` — `anyRequest().authenticated()`, with `actuator health/info` + springdoc (`/v3/api-docs/**`, `/swagger-ui/**`) permitted. Users are in-memory (`api`, `internal`), credentials from `app.auth.*` (env-overridable). The security filter chain only intercepts **inbound HTTP** — Oracle/Hikari, OTLP metric push, and container networking are unaffected. Because `*Client` calls loop back to this app over HTTP, `RestClientConfig` attaches the `internal` account's Basic credentials so self-calls pass the filter chain. A separate `@Profile("openapi")` permit-all chain keeps `generateOpenApiDocs` working.
