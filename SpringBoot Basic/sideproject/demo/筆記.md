...
- Repository : 處理資料的存取邏輯，負責和資料來源互動
  ...
  - `findAllById()`: 參數可以是 List , Set , Iterable，回傳 List； 對於不在找不到的id的處理為 continue ，理解為一句 : 把這些 ID中找得到的實體都回傳
...
- 單元測試
  - JUnit: java測試框架之一，提供許多功能，例如:
    ...
    - `@BeforeEach`:標註每個@Test執行前都要先執行這個標註的方法，通常用來做測試前的準備工作如初始化物件，搭配上MockitoAnnotations.openMocks(this)，可以在每個@Test執行前初始化Mock物件，若有`@ExtendWith(MockitoExtension.class)`則會產生衝突，可能是互相覆蓋到；適合用在全部測案都會用到且會修改的物件
    - `@BeforeAll`:在測試一開始執行這個標註的方法，方法需要為靜態方法static，適合用在全部測案都會用到且不會修改的物件
  - Mockito: java Mock框架，主要功能是創建、配置和驗證 mock 物件
    ...
    - `@Mock`: 指建立一個假的（或稱為模擬的）物件，用來替代被測試程式碼所依賴的真實物件。隔離被測試的程式，讓測試只關注這個單元的邏輯，模擬其他如外部服務API、資料庫或其他元件的影響。
    - `@InjectMock`: 將標註@Mock的物件注入到這個物件的對應欄位    
    - Stub「設定」或「打樁」這個模擬物件的行為，告訴它在特定情況下應該如何回應被測試的程式碼。`when(...).then(...)`: 定義當執行某Mock物件的方法時，指定要回傳甚麼值，可用於隔離開不同單元，例如: 測試Order時，呼叫accountClient的時候，指定應該回傳甚麼物件，不用真的去執行其他單元。
...
- Other
  ...
  - Transactional: 方法要是public才可以使用，因為算是一種AOP，而AOP只能攔截public方法；常見例子:從一個帳戶扣款，然後將款項存入另一個帳戶。
    ...
    - rollbackFor: 指定哪些例外會導致事務rollback，預設是 RuntimeException 和 Error 會rollback。
    - noRollbackFor: 指定哪些例外不會導致事務rollback。
  ...
  - 疑難雜症: `@ToString.Exclude`加在Entity中的OneToMany或ManyToOne的關聯上，否則會造成循環引用的問題，導致 StackOverflowError。