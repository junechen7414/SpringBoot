...
- Repository
  ...
  - JpaRepository<T, Type> 需要指定 Entity 類別 T 和該 Entity 的主鍵類型 ID。基本資料型態如 int 要使用 Integer，int 和 Integer 差在一個是包裝類別，有些方法可以使用，但也因此效率會比 int 差，且 int 不能為 null，可以避免 NullPointerException 的情況，另一方面 Integer 可以為 null，表示沒有值的狀態；java 會自動進行兩者間的轉換。
