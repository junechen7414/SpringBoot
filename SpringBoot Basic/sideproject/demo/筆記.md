...
- 單元測試
  - JUnit: java測試框架之一，提供許多功能，例如:
    ...
    - `@BeforeEach`:標註每個@Test執行前都要先執行這個標註的方法，通常用來做測試前的準備工作如初始化物件，搭配上MockitoAnnotations.openMocks()，可以在每個@Test執行前初始化Mock物件
  - Mockito: java Mock框架，主要功能是創建、配置和驗證 mock 物件
    ...
    - `ArgumentCaptor`:捕捉傳遞給 mock 物件方法的實際參數值，後續對這些參數進行更詳細的 assertions
- 實務情形:
