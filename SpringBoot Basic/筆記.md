...
# Other
*   **Transactional:** Spring 會在方法執行前後自動開啟和關閉。如果在方法執行過程中發生異常，會自動rollback。
*   **Optional:** 用於表示一個值可能存在或不存在。它可以避免空指標異常，是JPA Query Method findById()的預設返回值。
*   **application.properties:** 配置應用的行為、屬性。
    *   **常用配置:**
        *   **Server Port Configuration：**
            *   `server.port`：設定應用程式監聽的埠號。預設為 8080。

        *   **Database Connection Details：**
            *   `spring.datasource.url`：設定資料庫連線的 URL。
            *   `spring.datasource.username`：設定資料庫使用者名稱。
            *   `spring.datasource.password`：設定資料庫密碼。
            *   `spring.datasource.driver-class-name`：設定資料庫驅動程式類別名稱。

        *   **Spring Profile Settings：**
            *   `spring.profiles.active`：設定啟用的 Spring Profile。Profile 可以用於區分不同環境的配置（例如開發、測試、生產）。

            ```properties
            spring.profiles.active=dev
            ```

            *   可建立不同的 `application-{profile}.properties` 檔案，例如 `application-dev.properties` 和 `application-prod.properties`，來定義不同 Profile 的配置。

        *   **Logging Configurations：**
            *   `logging.level.root`：設定 root logger 的日誌等級。
            *   `logging.level.{package_name}`：設定特定套件的日誌等級。
            *   `logging.file.name`：設定日誌檔案的名稱。
            *   `logging.file.path`：設定日誌檔案的路徑。
        ...
        *   **JPA Configuration:**
            *   `spring.jpa.hibernate.ddl-auto`: 指定 Hibernate 如何根據 Entity 自動管理資料庫結構 (Schema)。常用選項：
                *   `validate`: 驗證目前的資料庫結構是否與 Entity 定義相符，若不符則拋出錯誤，不進行任何修改。
                *   `update`: 檢查資料庫結構與 Entity 定義的差異，並嘗試更新資料庫結構 (例如新增欄位、表格)。不刪除現有資料。
                *   `create`: 每次啟動時都刪除舊的資料表，然後重新建立新的資料表。資料會遺失。
                *   `create-drop`: 每次啟動時刪除舊表並建立新表，應用程式關閉時再次刪除所有表。
                *   `none`: 不進行任何 DDL 操作。
            *   `spring.jpa.show-sql`: 是否顯示 SQL 查詢。
            *   `spring.jpa.properties.hibernate.format_sql`: 是否格式化 SQL 查詢。

*   **範例：**[application.properties](demo/src/main/resources/application.properties)