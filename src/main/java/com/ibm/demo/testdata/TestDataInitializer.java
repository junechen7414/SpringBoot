...
public class TestDataInitializer implements CommandLineRunner {      
    ...
    public void run(String... args) throws Exception {
        log.info("=== 開始檢查測試資料初始化狀態 ===");
        
        // 檢查資料是否已存在，避免重複插入
        long accountCount = accountRepository.count();
        long productCount = productRepository.count();
        
        log.info("當前資料庫狀態 - 帳戶數量: {}, 商品數量: {}", accountCount, productCount);
        
        if (accountCount > 0 && productCount > 0) {
            log.info("測試資料已存在，跳過初始化流程");
            return;
        }
    ...
    private void initializeTestData() {
        ...
        log.info("儲存 {} 筆商品資料...", products.size());
        productRepository.saveAll(products);
        
        log.info("儲存 {} 筆帳戶資料...", accounts.size());
        accountRepository.saveAll(accounts);
        
        log.info("測試資料建立成功 - 帳戶: {}, 商品: {}", accounts.size(), products.size());
    }
}
